...
def home():
    ...
    cursor.execute(
        ...
        download_count FROM sheets ORDER BY
        download_count DESC LIMIT 12''')
    top_download = cursor.fetchall()

    # Top 12 uploaded sheets
    cursor.execute(
        '''SELECT id, filename, sheetname, composer, Instrument, created_at
        FROM sheets ORDER BY
        created_at DESC LIMIT 12''')
    latest_uploaded = cursor.fetchall()
    return render_template(
        'home.html',
        top_download=top_download, latest_uploaded=latest_uploaded)
...
def profile():
    ...
    if request.method == 'POST':
        ...
        if form_type == 'profile_update':
            ...
            if file and allowed_file(file.filename):
                ...
                cursor.execute(
                    "INSERT INTO sheets "
                    "(sheetname, filename, composer, instrument, uploader_id) "
                    "VALUES (?, ?, ?, ?, ?)",
    ...
    if active_tab == 'downloads':
        cursor.execute('''
                SELECT id, sheet_id, sheetname, filename, composer,
                instrument, download_at
                FROM downloads
                WHERE username = ?
                ORDER BY download_at DESC''', (username,))
    ...
    return render_template('profile.html', tab=active_tab, username=username,
                           email=email, sheets=sheets, downloads=downloads,
                           favourites=favourites, genres=genres)
...
def sheets():
    ...
    return render_template(
        'sheets.html', sheets=results, page=page, total_pages=total_pages)
...
def search():
    ...
    if query:
        ...
        cursor.execute('''
            SELECT id, filename, sheetname,
            composer, instrument, download_count
            FROM sheets
...
def download_file(sheet_id):
    ...
    cursor = db.cursor()
    cursor.execute(
        "UPDATE Sheets SET download_count = download_count + 1 WHERE id = ?",
        (sheet_id,)
        )
    ...
    if row:
        ...
        # Log donwload
        cursor.execute(
            '''INSERT INTO downloads (sheet_id, username, filename,'''
            '''sheetname, composer, instrument) VALUES (?, ?, ?, ?, ?, ?)''',
            (sheet_id, username, filename, sheetname, composer, instrument))

    db.commit()
    return send_from_directory(
        app.config['UPLOAD_FILES'], filename, as_attachment=True)
...
def favourite_file(sheet_id):
    ...
    cursor.execute('''
                   SELECT filename, sheetname, composer,
                   instrument FROM sheets WHERE id = ?''', (sheet_id,))
    row = cursor.fetchone()
    ...
    if row:
        ...
        filename, sheetname, composer, instrument = row
        cursor.execute('''
                       INSERT INTO favourites (sheet_id, username,
                       filename, sheetname, composer, instrument)
                       VALUES (?, ?, ?, ?, ?, ?)''',
                       (sheet_id, username, filename,
                        sheetname, composer, instrument))
...
def add_sheets():
    ...
    db = get_db()
    cursor = db.cursor()
    cursor.execute(sql, (sheetname, composer,
                         instrument, filename, uploader_id, download_count))
...
def preview_file(filename):
    return send_from_directory(
        app.config['UPLOAD_FILES'], filename, as_attachment=False)
...
def sheet_detail(sheet_id):
    ...
    cursor.execute('''
            SELECT s.id, s.filename, s.sheetname, s.composer, s.instrument,
                   s.download_count, g.name
            FROM sheets s
    ...
    if request.method == 'POST' and 'username' in session:
        ...
        if form_type == 'rating':
            ...
            cursor.execute('''
                           INSERT INTO comments (user_id, sheet_id, comment)
                           VALUES (?, ?, ?)''', (user_id, sheet_id, comment))
            db.commit()
    ...
    return render_template('sheet_detail.html', sheet=sheet,
                           comments=comments, avg_rating=avg_rating)
...
def edit_sheet(sheet_id):
    ...
    # Get the sheet
    cursor.execute('''
                   SELECT id, sheetname, composer, instrument, uploader_id
    ...
    if request.method == 'POST':
        ...
        if genre_id:
            cursor.execute("INSERT INTO SheetGenres (sid, gid) VALUES (?, ?)",
                           (sheet_id, genre_id))
    ...
    return render_template('edit_sheet.html', sheet=sheet, genres=genres,
...
def delete_sheet(sheet_id):
    ...
    cursor.execute("SELECT filename, uploader_id FROM sheets WHERE id = ?",
                   (sheet_id,))
    row = cursor.fetchone()
...
def delete_download(download_id):
    ...
    cursor.execute("SELECT username FROM downloads WHERE id = ?",
                   (download_id,))
    row = cursor.fetchone()
...
def delete_favourite(favourite_id):
    ...
    cursor.execute("SELECT username FROM favourites WHERE id = ?",
                   (favourite_id,))
    row = cursor.fetchone()
